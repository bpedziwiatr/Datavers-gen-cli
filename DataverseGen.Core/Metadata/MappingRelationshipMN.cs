...
public class MappingRelationshipMn : ICloneable, IMappingRelationship
{
	...
	public static MappingRelationshipMn Parse(
		ManyToManyRelationshipMetadata rel,
		string thisEntityLogicalName)
	{
		MappingRelationshipMn result = new();
		...
		if (rel.Entity1LogicalName == rel.Entity2LogicalName
			&& rel.Entity1LogicalName == thisEntityLogicalName)
		{
		...
		result.HybridName =
			MetadataNamingExtensions.GetProperVariableName(rel.SchemaName) + "_NN";

		result.PrivateName = "_nn" + rel.SchemaName.GetEntityPropertyPrivateName();
		result.ForeignKey =
			MetadataNamingExtensions.GetProperVariableName(result.Attribute.ToKey);
